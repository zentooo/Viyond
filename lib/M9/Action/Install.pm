...
my $clone_dir = M9::Config->get_value('m9_path') . "/repos";
...
sub install {
  ...
  if ( M9::InstallData::Metadata->load_all->[0]->{$repo_id} ) {
    M9::Action::Upgrade->upgrade($repo_id);
  }
  else {
    pathmk($repo_path);
    ...
    M9::InstallData::Metadata->add_entry($git_uri, $repository);
...
sub extract_to_vimfiles {
  ...
  M9::InstallData::Filelog->save($repo_id, \@destinations);
}
